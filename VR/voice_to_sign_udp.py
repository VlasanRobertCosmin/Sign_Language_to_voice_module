...
class SpeechToSignApp:
    ...
    def handle_udp_input(self, label, confidence=1.0):
        now = time.time()
        clean = ''.join(c for c in label.lower() if c.isalpha())
        if not clean:
            return
        
        # avoid rapid duplicates
        if clean == self.last_udp_word and (now - self.last_udp_time) < 1.0:
            return
        
        self.last_udp_word = clean
        self.last_udp_time = now
        
        print(f"\nUDP received: '{clean}' ({confidence*100:.1f}%)")
        added = self.enqueue_word(clean, source="udp")
        
        if added:
            self.status = f"UDP: {clean.upper()}"
    ...
    def run(self):
        print("\n" + "=" * 55)
        print("SPEECH TO SIGN - AVATAR + UDP")
        print("=" * 55)
