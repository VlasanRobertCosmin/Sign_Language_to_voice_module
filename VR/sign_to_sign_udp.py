"""
Real-Time ASL Signs Recognition with Voice Output + UDP Sender
==============================================================
Supports all model versions (v1, v2, v3)
Auto-detects model architecture from checkpoint.

Added:
- UDP sending of recognized sign
- Confidence included in UDP packet
- Sends JSON payload to external app / Unity / avatar module
"""
...
UDP_IP = "127.0.0.1"   # Change if your receiver is on another machine
UDP_PORT = 5005        # Must match your receiver port
SEND_UDP = True
SEND_ONLY_IF_CONFIDENT = True
UDP_CONFIDENCE_THRESHOLD = 0.30

udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)


def send_udp_prediction(sign, confidence, top_3=None):
    if not SEND_UDP:
        return
    
    if SEND_ONLY_IF_CONFIDENT and confidence < UDP_CONFIDENCE_THRESHOLD:
        return
    ...
    payload = {
        ...
        "top_3": [
            {"label": s, "confidence": float(c)} for s, c in (top_3 or [])
        ],
        "timestamp": time.time()
    }
    ...
    try:
        ...
        print(f"UDP SENT -> {sign} ({confidence*100:.1f}%)")
    except Exception as e:
        print(f"UDP Error: {e}")
...
def predict_sequence(model, label_encoder, classes, sequence):
    ...
    with torch.no_grad():
        ...
        top_3 = [
            (label_encoder.inverse_transform([idx.item()])[0], prob.item())
...
def run_demo():
    ...
    print("\n" + "=" * 60)
    print("ASL RECOGNITION WITH VOICE OUTPUT + UDP")
    print("=" * 60)
    print("Controls:")
    ...
    while True:
        ...
        if len(frame_buffer) >= max_frames:
            top_3 = predict_sequence(model, label_encoder, classes, list(frame_buffer))
            
            best_sign, best_conf = top_3[0]
            
            if voice_enabled and best_conf > 0.4 and best_sign != last_spoken:
                speak(best_sign)
                last_spoken = best_sign
                print(f"🔊 {best_sign} ({best_conf*100:.1f}%)")
            
            if udp_enabled and best_conf > UDP_CONFIDENCE_THRESHOLD and best_sign != last_sent:
                send_udp_prediction(best_sign, best_conf, top_3)
                last_sent = best_sign
        ...
        # UI
        h, w = frame.shape[:2]
        cv2.rectangle(frame, (10, 10), (460, 230), (0, 0, 0), -1)
        cv2.rectangle(frame, (10, 10), (460, 230), (255, 255, 255), 2)
        ...
        cv2.putText(frame, voice_status, (300, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.6, voice_color, 2)
        
        udp_status = "UDP ON" if udp_enabled else "UDP OFF"
        udp_color = (0, 255, 0) if udp_enabled else (0, 0, 255)
        cv2.putText(frame, udp_status, (300, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.6, udp_color, 2)
        
        progress = len(frame_buffer) / max_frames
        cv2.rectangle(frame, (20, 45), (280, 65), (50, 50, 50), -1)
        cv2.rectangle(frame, (20, 45), (20 + int(260 * progress), 65), (0, 200, 0), -1)
        cv2.putText(frame, f"{len(frame_buffer)}/{max_frames}", (120, 60),
        ...
        cv2.putText(frame, "r=rec | c=clear | p=predict | v=voice | u=udp | q=quit",
                    (10, h - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)
        
        cv2.imshow('ASL Recognition + Voice + UDP', frame)
        ...
        if key == ord('q'):
            ...
            print(f"UDP: {'ON' if udp_enabled else 'OFF'}")
            ...
            top_3 = predict_sequence(model, label_encoder, classes, sequence)
            
            best_sign, best_conf = top_3[0]
            
            if voice_enabled and best_conf > 0.3:
                speak(best_sign)
                last_spoken = best_sign
            
            if udp_enabled and best_conf > UDP_CONFIDENCE_THRESHOLD:
                send_udp_prediction(best_sign, best_conf, top_3)
                last_sent = best_sign
            
            print(f"Prediction: {best_sign} ({best_conf*100:.1f}%)")
